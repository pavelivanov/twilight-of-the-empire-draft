...
export function SetupScreen({ onCreated }: { onCreated: (draft: PublicDraft) => void }) {
  ...
  const validationError = useMemo(() => {
    ...
    if (players.some((player) => !player.trim())) return "Name all six players.";
    if (new Set(players.map((player) => player.trim().toLocaleLowerCase())).size !== 6) {
  ...
  async function submit(event: FormEvent) {
    ...
    setSubmitting(true);
    try {
      setDemoIdentity({ id: "creator", name: players[0]! });
      ...
      const draft = await api.createDraft({
        title,
        players: players.map((displayName) => ({ displayName })),
        config,
      });
    ...
    <main className="setup-shell">
      ...
      <form className="setup-layout" onSubmit={submit}>
        <section className="setup-intro">
          <span className="eyebrow">New six-player draft</span>
          <h1>
            Assemble the table.
            <br />
          ...
          <p>
            Name the players, choose the ruleset, and generate a reproducible balanced Milty pool.
        ...
        <div className="setup-form-panel">
          {step === 1 ? (
            <FieldGroup>
              ...
              {validationError && players.every(Boolean) && <FieldError>{validationError}</FieldError>}
            </FieldGroup>
          ) : (
            <FieldGroup>
              <FieldSet>
              ...
              <Field>
                ...
                <FieldDescription>More factions create more strategic choice; slices stay at nine.</FieldDescription>
              </Field>
